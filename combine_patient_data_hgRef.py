	...
	for i,line in enumerate(List):
		...
		if chrome not in Line_index_reference.keys() and len(Line_index_reference)==0:
			...
			if Line_index_reference[stripChr(List[i-1][2])][1] == '':
				Line_index_reference.update( { stripChr(List[i-1][2]) :  (( Line_index_reference[stripChr(List[i-1][2])][0],i)) } )
			else: 
				#hard coded
				Line_index_reference.update( { stripChr(List[i-8][2]) :  (( Line_index_reference[stripChr(List[i-8][2])][0],i)) } )
			Line_index_reference[chrome] = ((i,''))
...
def run_from_position(reference_genome_list,chrom_start,current_chrom,start,stop,file):
	...
	for i in reference_genome_list[start:stop]:
		...
		if tran_start <= chrom_start and chrom_start <= tran_end and current_chrom == chr_on_gene:	
...
def build(ref_fileName):
	...
	for name in glob.glob("./patient_data/*bedpe"):
		...
		with open("./Referenced_patient_data/"+name.split('patient_data/')[1].split('.')[0]+'.tsv',"wb") as file:
			file.write("geneA\tgeneA orientation\tgeneB\tgeneB orientation\tchr1\tposition\tchr1 orientation\tchr2\tposition\tchr2 orientation\n")
			for i,line in enumerate(filename):
				...
				#convert chromosomes (can't use previous function because they're stored as digits in string form, not "chr_")
				if columns[0] == "X" or columns[0] == "Y":
					chr1 = 23
				else: chr1 = int(columns[0])

				if columns[3] == "X" or columns[3] == "Y":
					chr2 = 24
				else: chr2 =int(columns[3])
				...
				if Aname != Bname and (Aname != ' ' and Bname != ' '):
					...
					if oriA != oriB and Avalues[0] == Bvalues[0]:
						Collection.append((Aname,Avalues[0],Bname,Bvalues[0],Avalues[1],chr1_pos,oriA,Bvalues[1],chr2_pos,oriB))
			...
			for k in Collection:
				line = str(k[0])+'\t'+str(k[1])+'\t'+str(k[2])+'\t'+str(k[3])+'\t'+str(k[4])+'\t'+str(k[5])+'\t'+str(k[6])+'\t'+str(k[7])+'\t'+str(k[8])+'\t'+str(k[9])
