...
import gene_finder
from compareagainstothers import *
import summarymatching
import re
import os
...
def generateDonorData(summaryfile, patient_fusion_directory, RNASummary, RNAData):
    D1 = summarymatching.patientDonorId(summaryfile, patient_fusion_directory)
    D2 = summarymatching.RNADonorId(RNASummary, RNAData)
...
def GroupCancers(ExonScores):
    ...
    Cancers = []
    os.system("mkdir cancer_type_groups")
    ...
    for name in glob.glob("./patient_fusions_with_exons/*"):
        ...
        if patientDonorId[fileheader][0] in inspect:
            ...
            if cancerType not in Cancers:
                os.system("mkdir cancer_type_groups/" + cancerType)
            os.system("cp " + name + " cancer_type_groups/" + cancerType + "/" + fileheader + "_" + str(column) + ".tsv")
...
def EvaluatePatientGroup(cancerType, ExonScores):
    ...
    for name in glob.glob("./cancer_type_groups/" + cancerType + "/*"):
        ...
        with open("./Results/" + cancerType + "/" + name.split(cancerType + "/")[1].split("_")[0] + ".tsv",
                  'w') as FileWithScore:
            for k, v in patientDict.iteritems():
                ...
                FileWithScore.write(
                    v[-1][0] + '\t' + v[-1][1] + '\t' + v[-1][2] + '\t' + str(v[-1][3]) + '\t' + v[-2] + '\n')
                CompareTo = UniqueGenes("./cancer_type_groups/" + cancerType + "/*", cancerType)
                columnsForPatientGene = [int(i[0].split('_')[1].split('.')[0]) for i in CompareTo[fileheader] if
                                         i[1] == patientsGene]
                ...
                for line in ExonScores:
                    ...
                    if gene_id == patientsGene:
                        ...
                        for x, z in zip(exonsPairs, Zscores):
                            FileWithScore.write(
                                str(exonNumber) + '\t' + str(x[0]) + ',' + str(x[1]) + ',' + str(z) + '\n')
...
def handleFileBuilding(ExonAnnotationFile_input, ExonScores_input, patient_fusion_directory_input,
    ...
    gene_finder.RNADonorId(ExonAnnotationFile, patient_fusion_directory)
...
def BuildResults(ExonScores):
    ...
    Cancers = [name.split("groups/")[1] for name in glob.glob("./cancer_type_groups/*")]
